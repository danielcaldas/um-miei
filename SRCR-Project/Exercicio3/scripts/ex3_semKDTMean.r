...
dataset <- read.csv("D:\\Dropbox\\SRCR\\Exercicio3\\Ex3_aleatorio.csv",header=TRUE,sep=";",dec=".")
...
fadiganet <- neuralnet(FatigueLevel ~ Performance.MAMean+Performance.MVMean+Performance.TBCMean+Performance.DDCMean+Performance.DMSMean+Performance.AEDMean+Performance.ADMSLMean+Performance.Task, dataset, hidden = c(6,6,5), threshold = 0.1)


# imprimir resultados
plot(fadiganet)

temp_teste  <-subset (total_5, select = c(Performance.MAMean,Performance.MVMean,Performance.TBCMean, Performance.DDCMean,Performance.DMSMean ,Performance.AEDMean, Performance.ADMSLMean, Performance.Task))
