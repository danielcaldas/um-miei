...
void Cone::write3DtoFile(string filename) {
	...
	int arraySize = (3 * ((nlados + ncamadas)*ncamadas));
	int tSize = (2 * ((nlados + ncamadas)*ncamadas));

	float* aVertex = (float*)malloc(arraySize*sizeof(float));
	float* aNormal = (float*)malloc(arraySize*sizeof(float));
